...
```{r}
#| label: Team-metric-plot
#| echo: false
#| message: false
#| warning: false

team_metric_plot <- function(team_value,
                             metric,
                             metric_label,
                             metric_is_percent = FALSE,
                             roll_n = 5,
                             data = SeasonSummaryTotalsDataGames_Plots) {

  dat <- data %>%
    filter(Team_Group == team_value) %>%
    mutate(Game_date = as.Date(Game_date, format = "%d-%m-%Y")) %>%
    ...
    mutate(
      metric_roll = slider::slide_dbl(
        value,
        ~ mean(.x, na.rm = TRUE),
        .before = roll_n - 1,
        .complete = TRUE
      ),
    metric_smooth = as.numeric(TTR::EMA(value, n = roll_n))
  )
...
max_val <- metric_limits %>% pull(max_val)
  ...
  min_date <- lubridate::floor_date(min(all_dates, na.rm = TRUE), "month")
  max_date <- lubridate::ceiling_date(max(all_dates, na.rm = TRUE), "month")

  team_avg <- mean(dat$value, na.rm = TRUE)
  ...
  team_colour <- ifelse(team_value == "All Oppo", Color_Away, Color_Home)

  ggplot(dat, aes(x = Game_date)) +
    geom_line(
      aes(y = value),
      linewidth = 0.4,
      alpha = 0.6,
    ...
    scale_x_date(
      ...
      breaks = seq(min_date, max_date, by = "1 month"),
      labels = scales::date_format("%b")
    ) +
    y_scale +
    labs(
...
```
```{r}
#| label: Team-metric-plot-wrapper
#| echo: false
#| message: false
#| warning: false

team_metric_two_up <- function(metric,
                               metric_label,
                               metric_is_percent = FALSE,
                               roll_n = 5,
                               team_left = Match_Home,
                               team_right = "All Oppo",
                               data = SeasonSummaryTotalsDataGames_Plots) {

  # Limit both plots to the same set of games (important for fair comparison)
  shared_games <- intersect(
    data %>% filter(Team_Group == team_left)  %>% pull(game_name),
    data %>% filter(Team_Group == team_right) %>% pull(game_name)
  )

  dat_shared <- data %>% filter(game_name %in% shared_games)

  cat("::: {.player-grid}\n\n")  # your CSS sets this to 2 cols

  # Left tile
  cat("::: {.callout-plain title='", team_left, "'}\n\n", sep = "")
  print(team_metric_plot(team_left, metric, metric_label, metric_is_percent, roll_n, data = dat_shared))
  cat("\n:::\n\n")

  # Right tile
  cat("::: {.callout-plain title='", team_right, "'}\n\n", sep = "")
  print(team_metric_plot(team_right, metric, metric_label, metric_is_percent, roll_n, data = dat_shared))
  cat("\n:::\n\n")
...
```{r}
#| label: team-metric-plot-function
#| echo: false
#| message: false
#| warning: false

team_plot_metric_dashboard <- function(metric,
                                       metric_label,
                                       metric_is_percent = FALSE,
                                       roll_n = 5,
                                       exclude_teams = NULL)  {
  base_dat <- SeasonSummaryTotalsDataGames %>%
    filter(
      Team_Group == Match_Home) %>%
    mutate(Game_date = as.Date(Game_date, format = "%d-%m-%Y")) %>%
        arrange(game_name, Game_date)

  if (!is.null(exclude_teams)) {
    base_dat <- base_dat %>% filter(!Team_Group %in% exclude_teams)
  }

  # overall average across all included teams (grey line)
  overall_avg <- base_dat %>%
  ...
  for (tm in teams) {
    ...
    cat("::: {.callout-plain title='", tm, "'}\n\n", sep = "")

    team_dat <- base_dat %>%
      filter(Team_Group == tm) %>%
      arrange(Game_date) %>%
      mutate(
        metric_roll = slider::slide_dbl(
          .data[[metric]],
    ...
    p_plot <- ggplot(team_dat, aes(x = Game_date)) +
      geom_line(
        aes(y = .data[[metric]]),
        linewidth = 0.7,
        alpha = 0.8,
      ...
      labs(
        title = tm,
        subtitle = paste0(
          "Solid = Team, Dashed = Rolling Avg (", roll_n,
          "), Grey = Overall Avg"
        ),
        x = NULL,
        y = metric_label,
...
# All Competitions Season Stats

```{r}
#| output: false
#| label: Season summary table setup averages
#posession and goals by team
SeasonSummaryAverageData <- compute_season_summary(SeasonData, group_vars = c("Team_Group")) 

SeasonSummaryAverageData <- SeasonSummaryAverageData %>%
...
SeasonSummaryAverageDataHalf <- compute_season_summary(SeasonData %>% mutate(Half = case_when(
  period == 1 ~ "First Half",
...
SeasonSummaryAverageDataHalf <- SeasonSummaryAverageDataHalf %>%
...
SeasonSummaryAverageDataPeriod <- compute_season_summary(SeasonData, group_vars = c("Game_Period", "Team_Group"))

SeasonSummaryAverageDataPeriod <- SeasonSummaryAverageDataPeriod %>%
...
SeasonSummaryAverageDataGameStateData <- compute_season_summary(SeasonData, group_vars = c("Game_State", "Team_Group"))

SeasonSummaryAverageDataGameStateData <- SeasonSummaryAverageDataGameStateData %>%
...
SeasonSummaryAverageDataGames <- compute_season_summary(SeasonData, group_vars = c("game_name", "Game_date", "Team_Group"))

SeasonSummaryAverageDataGames_Plots <- SeasonSummaryAverageDataGames

SeasonSummaryAverageDataGames <- SeasonSummaryAverageDataGames %>%
...
#posession and goals by team
SeasonSummaryTotalsData <- compute_summary(SeasonData, group_vars = c("Team_Group")) 

SeasonSummaryTotalsData <- SeasonSummaryTotalsData %>%
  select('Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'PKBox Touches' = PK_Box_Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFF Shots' = OFShots, 'SoT' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % xG Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, 'Long Passes' = Long_Passes, 'Long Ball Rate' = Long_Pass_Rate,  'Crosses' = Cross_Passes, 'Cross Success %' = Cross_Success, Corners, 'Short Corners' = Short_Corners, 'Possession Control' = Posession_Control, 'EPV % NPxG Con.')

SeasonSummaryTotalsDataHalfs <- compute_summary(SeasonData %>% mutate(Half = case_when(
  period == 1 ~ "First Half",
  period == 2 ~ "Second Half",
  period == 3 ~ "First Half Extra Time",
  period == 4 ~ "Second Half Extra Time",
...
SeasonSummaryTotalsDataHalfs <- SeasonSummaryTotalsDataHalfs %>%
  select(Half, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'PKBox Touches' = PK_Box_Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFF Shots' = OFShots, 'SoT' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % xG Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, 'Long Passes' = Long_Passes, 'Long Ball Rate' = Long_Pass_Rate,  'Crosses' = Cross_Passes, 'Cross Success %' = Cross_Success, Corners, 'Short Corners' = Short_Corners, 'Possession Control' = Posession_Control, 'EPV % NPxG Con.')

SeasonSummaryTotalsDataPeriods <- compute_summary(SeasonData, group_vars = c("Game_Period", "Team_Group"))

SeasonSummaryTotalsDataPeriods <- SeasonSummaryTotalsDataPeriods %>%
  select(Game_Period, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'PKBox Touches' = PK_Box_Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFF Shots' = OFShots, 'ONT Shots' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % xG Con.', 'Attempted Passes' = Attempted_Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, 'Long Passes' = Long_Passes, 'Long Ball Rate' = Long_Pass_Rate,  'Crosses' = Cross_Passes, 'Cross Success %' = Cross_Success, Corners, 'Short Corners' = Short_Corners, 'Possession Control' = Posession_Control, 'EPV % NPxG Con.')

SeasonSummaryTotalsDataGameState <- compute_summary(SeasonData, group_vars = c("Game_State", "Team_Group"))

SeasonSummaryTotalsDataGameState <- SeasonSummaryTotalsDataGameState %>%
  select(Game_State, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'PKBox Touches' = PK_Box_Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFF Shots' = OFShots, 'SoT' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % xG Con.', 'Attempted Passes' = Attempted_Passes,  'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, 'Long Passes' = Long_Passes, 'Long Ball Rate' = Long_Pass_Rate,  'Crosses' = Cross_Passes, 'Cross Success %' = Cross_Success, Corners, 'Short Corners' = Short_Corners, 'Possession Control' = Posession_Control, 'EPV % NPxG Con.')

SeasonSummaryTotalsDataGames <- compute_summary(SeasonData, group_vars = c("game_name", "Game_date", "Team_Group"))

SeasonSummaryTotalsDataGames_Plots <- SeasonSummaryTotalsDataGames

SeasonSummaryTotalsDataGames <- SeasonSummaryTotalsDataGames %>%
  select('Fixture' = game_name, Match_day, 'Team' = Team_Group, 'Possession %' = Possession, 'On Ball Time' = In_play_time, Touches, 'PKBox Touches' = PK_Box_Touches, 'Field Tilt %' = Field_Tilt, 'Total Shots' = TotalShots, 'OFF Shots' = OFShots, 'SoT' = OTShots, '6yrd box Shots' = SixYard_Box_Shots, 'PK Box Shots' = PK_Box_Shots, 'Outside Box Shots' = Out_Box_Shots, BCs, Goals, xG, NPxG, xGOT, EPV = TeamEPV, 'EPV % xG Con.', 'Attempted Passes' = Attempted_Passes,  'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, 'Long Passes' = Long_Passes, 'Long Ball Rate' = Long_Pass_Rate,  'Crosses' = Cross_Passes, 'Cross Success %' = Cross_Success, Corners, 'Short Corners' = Short_Corners, 'Possession Control' = Posession_Control, 'EPV % NPxG Con.')

SeasonSummaryTotalsDataGames_Plots <- SeasonSummaryTotalsDataGames_Plots %>%
  mutate(Aerial_Success = Aerial_Won / Aerial_total,
         Duel_Success   = Ground_Duels_Won / Ground_Duels_Total,
         'xG/Shot' =  xG/TotalShots,
  Shot_Accuracy = OTShots/TotalShots,
  Shot_Quality = (xGOT - xG)/xG
  )
...
## All Competitions Season Stats {.tabset}
...
MatchSummarySeasonTable <- SeasonSummaryTotalsData %>%
...
```
```{r}
#| title: Season Summary by Half
#| results: asis
#| label: Season summary tables halfs

MatchSummarySeasonHalfsTable <- SeasonSummaryTotalsDataHalfs %>%
  ...
  tab_style(
    ...
    style = list(cell_text(weight = "bold", size = 16))) %>%
    tab_style(
    locations = cells_row_groups(groups = unique(SeasonSummaryTotalsDataHalfs$Half)),
...
```
```{r}
#| title: Season Summary by Game Period
#| results: asis
#| label: Season summary tables P

MatchSummarySeasonPeriodTable <- SeasonSummaryTotalsDataPeriods %>%
  ...
  tab_style(
    ...
    style = list(cell_text(weight = "bold", size = 16))) %>%
    tab_style(
    locations = cells_row_groups(groups = unique(SeasonSummaryTotalsDataPeriods$Game_Period)),
    style = cell_text(weight = "bold", size = 16, color = "white")
  ) %>%
  tab_style(
    style = list(cell_fill(color = "red4"),
...
```
```{r}
#| title: Season Summary by Game State
#| results: asis
#| label: Season summary tables GS

MatchSummarySeasonGameStateTable <- SeasonSummaryTotalsDataGameState %>%
  ...
  tab_style(
    ...
    tab_style(
    locations = cells_row_groups(groups = unique(SeasonSummaryTotalsDataGameState$Game_State)),
    style = cell_text(weight = "bold", size = 16, color = "white")
  ) %>%
...
``` {r}
#| title: Season Summary By Fixture
#| results: asis
#| label: Season summary tables fix

MatchSummaryGameNameData <- SeasonSummaryTotalsDataGames %>%
...
PassConnections <- PassConnections %>%
  ...
  select(Passer = playerId, Receiver = receiver, Passer_Team = teamId, Receiver_Team = receiver_team, Successful_Passes, BCs, Key_Passes, Prog_Passes, EPV = EPVvalue, x, y, Endx, Endy, period) 

PassConnectionsEPV <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1)

PassConnectionsEPV2nd <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1)
...
PassConnectionsKP2nd <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1,
         Key_Passes == 1)
...
PassConnectionsPP2nd <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1,
         Prog_Passes == 1)

PassConnectionsBC <- PassConnections %>%
  filter(EPV >= 0.020,
         Successful_Passes == 1,
         BCs == 1)
...
EPV_PassesFT <- PassConnectionsEPV %>%
  ...
  #formatting
  labs(title = paste("Successful Passes with threat higher than 0.2 EPV"), 
...
EPV_PassesFT <- PassConnectionsPP %>%
  ...
  #formatting
  labs(title = paste("Successful Progressive Passes with threat higher than 0.2 EPV"), 
       subtitle = paste0("Increased Red = Higher Threat (EPV)"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
EPV_PassesFT <- PassConnectionsKP %>%
  ...
  #formatting
  labs(title = paste("Successful Key Passes with threat higher than 0.2 EPV"), 
       subtitle = paste0("Increased Red = Higher Threat (EPV)"),
       caption =  My_caption) +
  theme(plot.background = element_rect(colour = "snow", fill = "snow"),
        plot.caption = element_markdown(),
...
##### Metric trends {.tabset}

```{r}
#| title: Threat Conversion
...
team_metric_two_up("EPV % NPxG Con.", "Threat Conversion", FALSE)
...
team_metric_two_up("TeamEPV", "Attacking Threat", FALSE, roll_n = 5)
...
team_metric_two_up("xG", "Expected Goals", FALSE, roll_n = 5)
...
team_metric_two_up("xGOT", "Expected Goals on Target", FALSE, roll_n = 5)
...
team_metric_two_up("Shot_Accuracy", "Shot Accuracy", TRUE, roll_n = 5)
...
team_metric_two_up("Shot_Quality", "Shot Quality", FALSE, roll_n = 5)
...
team_metric_two_up("Aerial_Success", "Aerial Success", TRUE, roll_n = 5)
...
team_metric_two_up("Duel_Success", "Duel Success", TRUE, roll_n = 5)
...
team_metric_two_up("Posession_Control", "Possession Control", FALSE, roll_n = 5)
...
team_metric_two_up("Prog_Passes", "Progressive Passes", FALSE, roll_n = 5)
...
#| title: Season Summary
#| results: asis
#| label: Season summary tables P90


MatchSummarySeasonTable <- SeasonSummaryAverageData %>%
  gt(groupname_col = "Team") %>%
  cols_hide('On Ball Time') %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
...
#| title: Season Summary by Half
#| results: asis
#| label: Season summary tables halfs p90

MatchSummarySeasonHalfsTable <- SeasonSummaryAverageDataHalf %>%
  gt(rowname_col = "Team", groupname_col = "Half") %>%
  cols_hide('On Ball Time') %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
  ...
  tab_style(
    ...
    style = list(cell_text(weight = "bold", size = 16))) %>%
    tab_style(
    locations = cells_row_groups(groups = unique(SeasonSummaryAverageDataHalf$Half)),
    style = cell_text(weight = "bold", size = 16, color = "white")
  ) %>%
    tab_style(
    style = list(cell_fill(color = "red4"),
...
#| title: Season Summary by Game Period
#| results: asis
#| label: Season summary tables P p90

MatchSummarySeasonPeriodTable <- SeasonSummaryAverageDataPeriod %>%
  gt(rowname_col = "Team", groupname_col = "Game_Period") %>%
  cols_hide('On Ball Time') %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
  ...
  tab_style(
    ...
    style = list(cell_text(weight = "bold", size = 16))) %>%
    tab_style(
    locations = cells_row_groups(groups = unique(SeasonSummaryAverageDataPeriod$Game_Period)),
    style = cell_text(weight = "bold", size = 16, color = "white")
  ) %>%
  tab_style(
    style = list(cell_fill(color = "red4"),
...
#| title: Season Summary by Game State
#| results: asis
#| label: Season summary tables GS p90

MatchSummarySeasonGameStateTable <- SeasonSummaryAverageDataGameStateData %>%
  gt(rowname_col = "Team", groupname_col = "Game_State") %>%
  cols_hide('On Ball Time') %>%
  cols_align(align = "right", columns = everything()) %>%
  tab_header(
  ...
  tab_style(
    ...
    tab_style(
    locations = cells_row_groups(groups = unique(SeasonSummaryAverageDataGameStateData$Game_State)),
    style = cell_text(weight = "bold", size = 16, color = "white")
  ) %>%
...
#| title: Season Summary By Fixture
#| results: asis
#| label: Season summary tables fix p90

MatchSummaryGameNameData <- SeasonSummaryAverageDataGames %>%
  mutate(Match_day = as.numeric(Match_day)) %>%
  filter(!Match_day == 0) %>%
  arrange(Match_day)
...
#| message: false
#| warning: false
#| label: player passing profiles setup P90
...
```{r}
#| title: Threat Conversion
#| label: Threat Conversion Season P match p90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: Attacking Threat
#| label: Home threat graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: xG
#| label: xG graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: xGOT
#| label: xGOT graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: Shot Accuracy
#| label: Shot a graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: Shot Quality
#| label: shot q graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: ADs
#| label: Home AD graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: DAs
#| label: Home DA graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: PC
#| label: PC graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
...
```{r}
#| title: Prog Passes
#| label: Home Pps graph Season P match P90
#| echo: false
#| message: false
#| warning: false
#| results: asis
